...
#         self.string = 'temp' + str(len(temps)+1)
...
def p_expression_group(t):
    ...
    t[0].addr = '(' + str(t[2]) + ')'
    # quadruples.append(('(' + str(t[0].addr) + ')'))
...
def p_declarationList_idList_type(t):
    ...
    for n in str(t[1]).split(','):
        if t[3] in type_names:
            type_names[t[3]].append(n)
        else:
            type_names[t[3]] = [n]
...
def p_declarationList_decidtype(t):
    ...
    for n in str(t[3]).split(','):
        if t[5] in type_names:
            type_names[t[5]].append(n)
        else:
            type_names[t[5]] = [n]
